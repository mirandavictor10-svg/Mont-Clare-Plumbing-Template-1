...
const badges = company.trustBadges.map((b) => ({
  ...b,
  icon: platformIcons[b.platform],
}));

const BadgeItem = ({ label, sublabel, icon }: typeof badges[number]) => (
  <div className="flex items-center gap-2.5 px-8 flex-shrink-0">
    <span className="flex-shrink-0">{icon}</span>
    <span className="leading-tight">
      <span className="block text-sm font-heading font-bold text-foreground">{label}</span>
...
const TrustStrip = () => (
  <section aria-label="Trust credentials" className="bg-card border-y border-border py-4 overflow-hidden relative">
    {/* Edge fades */}
